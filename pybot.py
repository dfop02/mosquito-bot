...
async def on_message_delete(message):
    COLD_ID = 725525536478068788

    # Collect user who deleted the message
    async for entry in message.guild.audit_logs(action=discord.AuditLogAction.message_delete, limit=1):
        deleted_by = entry.user

    # Print who deleted a message on main channel
    # doesnt work well if user delete own message
    if not message.author.bot and deleted_by.id != message.author.id or deleted_by.id == COLD_ID:
        response = 'The {} deleted {}'.format(
            deleted_by.name,
            'my msg!' if message.author.name == bot.user.id else f'a msg from {message.author.name}!'
        )
...
# go to a voice channel and say something
